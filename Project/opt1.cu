    ...
    */
    ...
    if(W_index < W_out && H_index < H_out){
        ...
        for(int i = 0; i < C; i++){
            for(int j = 0; j < K; j++){                    
                for(int k = 0; k < K; k++){
                    temp += in_4d(batch_index, i , H_index * S + j, W_index * S + k) * mask_4d(out_channel_index, i , j , k);  // because of the stride
                }
            }
        }
...
__host__ void GPUInterface::conv_forward_gpu_prolog( float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    */
    ...
    for(int  i =0 ; i < num_streams; i++){
        int offset = i * B * C * H * W / num_streams;

        cudaMemcpyAsync(*device_input_ptr + offset, host_input + offset, sizeof(float) * B * C * H * W / num_streams, cudaMemcpyHostToDevice, streams[i]);

    }
    dim3 Dimblock( Tile_widht, Tile_widht, 1);
    dim3 Dimgrid((B-1)/num_streams + 1, M, gridW * gridH);    

    for(int  i =0 ; i < num_streams; i++){

        
        conv_forward_kernel<<<Dimgrid, Dimblock, 0, streams[i]>>>(*device_output_ptr + i * B * M * ((H - K)/S + 1) * ((W - K)/S + 1) / num_streams , *device_input_ptr + i * B * C * H * W / num_streams , NULL, B, M, C, H, W, K, S);

    }
    for(int  i =0 ; i < num_streams; i++){
        int offset = i * B * M * ((H - K)/S + 1) * ((W - K)/S + 1) /num_streams ;

        cudaMemcpyAsync(host_output + offset, *device_output_ptr + offset, sizeof(float)* B * M * ((H - K)/S + 1) * ((W - K)/S + 1)/num_streams, cudaMemcpyDeviceToHost, streams[i]);
    }
